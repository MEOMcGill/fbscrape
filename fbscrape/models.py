...
class Query:
    ...
    ENDPOINT_REGISTRY: ClassVar[dict] = {
        ...
        "Search": {
            ...
            "modes": {
                # Hybrid only (as are all endpoints). Date bounds are enforced
                # server-side via the
        ...
        "GroupTimeline": {
            # `handle` accepts either a vanity group handle (e.g. "392585550772135")
...
class ScrapeOutcome:
    ...
    # inline in `data` (single-shot endpoints), and `num_records`
