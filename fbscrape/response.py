...
class ResponseInterceptor:
    ...
    def __init__(self):
        ...
        # never accumulated in RAM. None => append to `self.posts`
        ...
        # of newly-added records — from the hybrid pagination loops, which add
        # posts themselves (and, in principle, from the legacy auto-extract path
        # on `intercept_response`, which scrape methods disable via
        ...
        # so hybrid replays will not pollute these values.
        ...
        # off-date-range posts. Defaults True (the legacy auto-extract path);
        # every scrape method sets it False. Token tracking, viewer detection,
        # and the network_capture all keep working regardless of this flag.
    ...
    async def intercept_response(self, response: Response):
        ...
        try:
            ...
            if self.extract_posts:
                ...
                if parsed:
                    # Legacy auto-extract path (posts pulled straight off
                    # intercepted responses). Every current scrape method sets
                    # extract_posts=False and fires the hook from its own loop
                    # after add_posts, so this branch is not taken during a
                    # normal scrape.
    ...
    def add_posts(self, posts: list[dict]) -> list[dict]:
        ...
        for post in posts:
            ...
            if self.post_sink is not None:
                ...
                self.posts.append(post)     # accumulate in RAM (single-shot)
            self.post_count += 1
            added.append(post)
        return added
