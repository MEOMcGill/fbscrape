...
class FacebookScraper:
    ...
    async def user_timeline(
        ...
        **params,
        ...
        Args:
            ...
            mode: scrape strategy. "hybrid" (page.request POST-driven, no
                  scroll-induced DOM growth) is the only supported mode and
                  the default.
            ...
            resume_from: Path (`.json` or `.json.gz`) to a previously-saved
                  `ScrapingResult` to resume from. The saved `last_cursor`
        ...
        # Resume support. Cursor goes to leg 0 only (post cursor_reset legs
        # start fresh with adjusted end_date by design). Saved post_ids seed
        # dedup on every leg so re-served bootstrap-edge posts don't
        # duplicate, AND go into the cross-leg `seen_post_ids` accumulator
        # below so flatten-based dedup catches anything the interceptor misses.
        initial_cursor = ""
    ...
    async def group_timeline(
        ...
        **params,
        ...
        Args:
            handle: Vanity group handle (e.g. "392585550772135") or the
