...
24. **Resume-state sidecar makes `--continue` reads O(summary), not O(file).** KDD 22's `_stream_resume_state` still ijson-walks the *entire* posts payload (hundreds of MB) just to recover the resume `last_cursor` + recent `post_id`s. On a slow host that read takes minutes per file and, being GIL-bound, starves the event loop so workers can't open browsers until every target's read finishes (observed: ~18 min of "workers initialized but blocked" on a 71-handle batch on a ThinkStation P340 Tiny, vs. ~2 min on an M4 Pro). The fix persists that derived state at save time — both values are already in memory — into a tiny `<stem>.resume.json` companion (`fbscrape/resume_sidecar.py`). `scraper._read_resume_state` tries the sidecar first (O(1) `json.load` of a KB-sized file) and falls back to `_stream_resume_state` only when the sidecar is missing / stale / malformed; all three resume callers (`user_timeline` / `group_timeline` / `comments_list`) go through it. The writer (`write_sidecar`) is called from `cli._finalize_continue_result` — the single save path for all three resumable endpoints (UserTimeline / GroupTimeline / CommentsList), fresh runs and `--continue` alike — and is best-effort (a sidecar-write failure is logged, never fails the save). **Consistency validator:** the sidecar records the main file's `source_size` + `source_mtime`; the reader trusts it only if `os.stat` still matches, so any out-of-band edit silently invalidates it (→ full-parse fallback). **Two deliberate caps** (both in `resume_sidecar.py`): `post_ids` keeps the **last 150** in collection order — this is a semantic relaxation from KDD 22's exhaustive id set to *boundary-only* dedup, sized to exceed FB's worst-case re-serve window `max_no_progress_streak × pagination_count` (GroupTimeline 30×3 = 90) so the `NoNewPostsStreak` backstop still fires; `cursors` keeps the head (`last_cursor`, the only entry consumed today) plus the **last 100** per-post cursors, stored inert for a future unstick-via-ladder feature. Self-healing: a file with no sidecar (legacy corpus, or pre-feature) pays one slow fallback parse, then its next save writes a sidecar — so the *second* `--continue` is fast regardless. The existing corpus can be retrofitted up-front with `fbscrape utils backfill-sidecars <paths>` (`resume_sidecar.backfill_file` / `collect_post_files` — a single streaming ijson pass per file producing the identical payload; `--dry-run` / `--force` / `-q` flags). On-disk format of the main files is unchanged.

25. **`--continue` merge streams prior→new disk-to-disk (bounded memory).** The post-scrape merge in `cli._finalize_continue_result` used to `json.load` the *entire* prior file and concatenate `prior + new` in memory. A 300–400 MB gzip becomes 8–15 GB of live Python objects, and several merges overlapping (dispatched eagerly per completed handle onto the default thread pool) blew past the container's 48 g `mem_limit` — the cgroup OOM-killer killed the whole run mid-scrape (observed 2026-06-03: `fbscrape` RSS hit 46 GiB; `CONSTRAINT_MEMCG`, host had 60 GiB free). `fbscrape/merge.py` `stream_merge_and_save` rewrites the file as a stream: prior records flow one at a time via `ijson.items(..., use_float=True)` to a temp gzip, the (bounded, `--max-posts`-capped) new records are appended, then `os.replace` (atomic, like `save`). Peak memory is `O(one prior record + new leg)` — *independent of prior-file size*. **`use_float=True` + a `Decimal` json default are load-bearing**: ijson parses JSON numbers as `Decimal`, which `json.dumps` can't serialize — without the fix every real record (all carry numeric fields) raises `TypeError` and silently drops the prior to the new-only fallback (caught by a real-data smoke, not the string-only unit tests). The merge computes two byproducts in the same pass: the **resume sidecar** from the merged tail (KDD 24 — must be computed here, not via `write_sidecar` on the post-merge result, since after a streaming merge `result.data` holds only the *new* records), and the **auto-unstick** cursor on `no_new_posts_streak` legs (streaming flatten + the shared `merge._unstick_select`, identical selection to `cli._find_unstick_cursor`). **Failure isolation:** an unreadable/corrupt/truncated prior (e.g. `EOFError` on a half-written gzip) is caught, the partial temp discarded, and the new leg saved new-only — never lost, never a half-merged destination. A superseded legacy uncompressed `.json` prior is removed after a successful `.json.gz` write. A small `asyncio.Semaphore(MERGE_CONCURRENCY=3)` (`cli._finalize_guarded`) caps concurrent merges — cheap insurance against disk/CPU thrash now that each merge is memory-bounded, no longer the load-bearing fix. The remaining memory floor is the *new* leg held in RAM during the scrape (bounded by `--max-posts`); shrinking that is a separate change (incremental result streaming). Output is valid JSON, one record per `data`-array line (compact, not `indent=2` — far smaller on these files; all readers are whitespace-agnostic), so newly-merged files differ in formatting from `save()`-written ones but parse identically.
...
├── merge.py             # Streaming --continue merge (prior+new disk→disk, bounded memory) (KDD 25)
...
├── resume_sidecar.py    # Resume-state sidecar (write/read <stem>.resume.json) — fast --continue (KDD 24)
...
# Backfill resume-state sidecars onto existing scrape outputs so the next
# --continue is fast (KDD 24). Accepts file(s) or directories; one streaming
# ijson pass per file; skips current sidecars (unless --force) and non-resumable
# endpoints. Run on a quiescent corpus (a file the live scrape re-saves
# mid-backfill invalidates its sidecar). --dry-run previews.
