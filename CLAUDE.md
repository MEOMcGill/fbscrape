...
- **No server-side date filter.** GCFRSPQ variables carry no `beforeTime` / `afterTime` — date bounding is purely client-side via parser-extracted `creation_time` vs. `start_unix`. `end_date` is therefore advisory in this endpoint (it only bounds the stop check; FB always returns from the current head of feed when cursor is null). Key Design Decision 12 (always-set `beforeTime`) does NOT apply.
