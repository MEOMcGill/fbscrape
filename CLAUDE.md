...
- Navigates, then scrolls to provoke a natural PCTFRQ and captures its form body + headers as a replay template (via `ResponseInterceptor.latest_pctfrq_request`). The capture **re-scrolls while it waits** (`_hybrid_wait_for_template(rescroll=True)`): a single bootstrap scroll often fails to fire the feed-refetch under load (page not yet rendered, or one viewport isn't past FB's SSR posts), so it keeps scrolling deeper (`scrollBy` is relative) every few seconds until the query fires or `template_capture_timeout` (default 45 s) elapses. Single-shot endpoints (PageTransparency / ProfileAuthenticity) don't scroll — they wait for any natural GraphQL POST (`rescroll=False`).
...
- `ScrapeOutcome(result, data, time_started, time_taken, last_cursor, post_count, spill_path)` — Query-agnostic outcome from `BrowserSession`. Doesn't know which Query produced it. Records live either inline in `data` (single-record endpoints like PageTransparency; manual mode) or, for write-on-parse paginated scrapes, in the `spill_path` `.jsonl.gz` with `data` empty and `post_count` set (`num_records` covers both).
- `ScrapingResult(query, result, data, time_started, time_taken, last_cursor, post_count, spill_path)` — final result. Composed by `Worker` via `ScrapingResult.from_outcome(query, outcome)` so the canonical Query is constructed exactly once and never rebuilt downstream. `num_records` = `post_count` (when records are spilled to disk, write-on-parse) else `len(data)`; `iter_posts()` streams from the spill or iterates inline `data`. **Saved as one-post-per-line JSONL** (`<stem>.jsonl.gz`, KDD 24): each line `{query, result, …, last_cursor, data: <one record>}`. The CLI `flatten` / `download-media` loaders are dual-format (`jsonl_store.load_scrape_file`) — they also read the legacy whole-file envelope (`"data": [...]` / pre-rename `"posts": [...]`).
...
`login()` has a **transient-nav chokepoint**: its helpers (`login_with_cookies`, `check_logged_in`) issue several *bare* `page.goto` / `page.reload` calls, and a renderer/network flake there raises a raw playwright `TimeoutError` / `TargetClosedError`. That type is NOT in `Worker.execute_task`'s typed-retry set, so it would escape untyped, get set on the future, and re-raise through `FacebookScraper.user_timeline`'s `await future` → `gather()`'s `yield await c` — tearing down the **entire batch** (one bad navigation killing every other handle; observed in production 2026-06-02). `login()` wraps its orchestration in a narrow `except (TimeoutError, TargetClosedError)` that reclassifies those into `TransientLoginError` (worker rotates + retries, account stays `active=True`), plus a second `except PlaywrightError` that reclassifies a *base* playwright `Error` **only** when its message matches a known nav/network-abort marker (`_TRANSIENT_NAV_MARKERS`: `NS_BINDING_ABORTED`, `NS_ERROR_NET_*`, `NS_ERROR_*PROXY*`, `net::ERR_`, …) — observed in production 2026-06-04 as one worker dead-churning `Page.reload: NS_BINDING_ABORTED` → `task_failed` every ~90s without rotating, because the error was untyped. Any other base `Error` re-raises untouched (no broad swallowing). The catch is deliberately narrow — checkpoint / ban / disabled typed exceptions and the terminal `FailedLoginError` pass straight through. Residual edge: 3 consecutive transients on one handle still raise `RetryBudgetExhaustedError` (untyped → would still kill the batch); full per-handle isolation in the scrape loop is a separate, not-yet-applied change.
...
24. **One-post-per-line JSONL is the on-disk format; `--continue` appends.** Scrape outputs are `<stem>.jsonl.gz` where **each line is a self-contained envelope carrying a single record**: `{query, result, time_started, time_taken, last_cursor, data: <one Story>}` (`fbscrape/jsonl_store.py`). This replaced the monolithic `{… "data": [all posts] …}` envelope (and retired the KDD-22 sidecar and KDD-25 streaming merge). Why: with an envelope, every `--continue` leg had to **rewrite the whole prior file** (load it, concat `prior+new`, re-dump) — a 300–400 MB gzip becomes 8–15 GB of live Python objects, and several overlapping merges OOM-killed the run at the 48 g cgroup limit (observed 2026-06-03: RSS hit 46 GiB, `CONSTRAINT_MEMCG`). JSONL makes `--continue` an **append** (`gzip.open(path,"at")` adds a new member; readers decompress all members) — O(new leg), prior never loaded. Per-line metadata semantics: `query`/`time_started` are constant and ride every line; `last_cursor` is the page cursor of that post's batch; `result`/`time_taken` are `null` mid-leg and stamped on the leg's **final line** (the last line of the file is authoritative for leg status). `ScrapingResult.save()` writes JSONL via `JsonlPostWriter` (a 1-post buffer lets it stamp the terminal line without holding the leg), gzip default, atomic temp+`os.replace` on a fresh write, `append=True` for `--continue`. **Resume** reads the JSONL **tail** (`jsonl_store.read_resume_tail` — decompress + a bounded rolling window of the last ~150 lines → `last_cursor` + recent `post_id`s for cross-leg dedup; no sidecar). `scraper._read_resume_state` dispatches: JSONL → tail-read, legacy envelope → `_stream_resume_state`. **Dual-format everywhere**: `looks_like_jsonl` (a JSONL line's `data` is a single dict/null; an envelope's `data`/`posts` is a *list*) routes readers; `jsonl_store.load_scrape_file` returns `(query, records)` for either format, used by `flatten` / `download-media` / `unstick-cursor`. **Auto-unstick** on a `no_new_posts_streak` `--continue` leg appends a trailing status-only line (`data: null`) carrying a deeper (rank-3 oldest cursored) cursor, so the next tail-read resumes past the dedup wall. **Migration**: a legacy envelope prior is converted to JSONL on first `--continue` (self-healing); `fbscrape utils convert-to-jsonl <paths>` (`jsonl_store.convert_envelope_to_jsonl`, one streaming ijson pass; `--dry-run` / `--delete-original`) does the corpus up-front. Decimal-safety (`_json_default`) is load-bearing — ijson yields `Decimal`, which `json.dumps` can't serialize. Output is compact (no `indent`); all readers are whitespace-agnostic.

25. *(superseded by KDD 24)* — the disk-to-disk streaming merge that briefly replaced the in-memory rewrite is gone; JSONL `--continue` is a plain append, so there is no merge to stream. The `mem_limit` / OOM history that motivated it lives in KDD 24.
...
├── jsonl_store.py       # One-post-per-line JSONL I/O: writer/append, dual-format readers, resume tail-read, legacy→JSONL converter (KDD 24)
...
# Convert legacy whole-file envelope outputs to one-post-per-line JSONL (KDD 24).
# Writes <stem>.jsonl.gz next to each legacy <stem>.json{,.gz}; one streaming
# ijson pass per file; skips files already in JSONL. --continue also migrates a
# legacy prior on first resume, so this is an optional up-front bulk pass.
...
- **`flatten --concat`: streaming / bounded-memory path.** Today `--concat` (`cli.py`, `mode == 'CONCAT'`) accumulates *every* record from *every* input into one `all_rows` list, then builds a single `pl.json_normalize` DataFrame — O(total corpus) in RAM. Per-file mode is no better for a single huge input: `load_scrape_file()` materializes one whole file's records. On a real master-seedlist corpus (493 `.jsonl.gz`, ~28 GB compressed, individual files up to 2.8 GB) this OOMs an 18 GB machine outright; the workaround was an external script that streams `iter_posts()` (the generator primitive, never materializes a file), flattens post-by-post, flushes parquet shards every N rows, then combines via `pl.concat([scan_parquet(s) ...], how='diagonal_relaxed').sink_parquet(out, compression='zstd')`. Peak RSS stayed ~1 GB (brief ~5 GB spike on the largest batch). Fold this into `flatten` itself: stream inputs through `iter_posts`, batch-normalize to a bounded buffer, and either write shards + streaming-concat or sink row-groups directly. `diagonal_relaxed` handles the per-file schema drift that the current single-`json_normalize` gets "for free". Keep the existing in-memory path for small inputs; switch to streaming above a size/row threshold (or always — `sink_parquet` is strictly safer). See `docs/proposals/speed_and_memory.md`.