...
import json
from .post_scraper import InstaPostScraper
...
def consumer(username, password):
    def callback(ch: BlockingChannel, method, properties, body):
        if insta_scraper.scraped_so_far > batch_size:
            print(f"{batch_size} accounts scraped consecutively. Resting for {rest_time} seconds")
            ch.basic_nack(delivery_tag=method.delivery_tag)
            # insta_scraper.insta_session.page.goto("https://nytimes.com")
            time.sleep(rest_time)
            insta_scraper.scraped_so_far = 0
        else:
            ...
            metadata = insta_scraper.scrape_seed(
                message, ch
            )

            if metadata['result'] not in insta_scraper.retry_cases:
    ...
    insta_scraper = InstaPostScraper(
                 headless,
                 mobile,
                 log_in_fresh,
                 auth_dir,
    ...
    insta_scraper.identify_country_from_which_you_are_scraping()

    if insta_scraper.ip_country is None:
        input('VPN ok?')
    elif insta_scraper.ip_country == 'CA':
        raise Exception("You are scraping from Canada -> C-18 Meta news ban may mask certain accounts")
    else:
        print(f"scraping from {insta_scraper.ip_country}")

    insta_scraper.create_playwright_instance()
    insta_scraper.initialize_insta_session()
    insta_scraper.log_in_if_necessary()
