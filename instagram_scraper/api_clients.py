...
def get_seedlist_new():
    ...
    target_url = urljoin(API_BASE_URL, "/phh/seedlist?query=Platform:instagram")
    with requests.session() as session:
...
def get_crawler_histories() -> list[dict]:
    ...
    response = requests.get(urljoin(API_BASE_URL, "/phh/get/crawler_history?query=Platform:instagram"), headers=headers,
                            verify=True)
...
def get_gaps_api() -> list[dict]:
    ...
    response = requests.get(urljoin(API_BASE_URL, "/phh/historical_seedlist?query=Platform:instagram"), headers=headers,
                            verify=True)
