...
ALL_PAIRS = [
    ...
    ("GroupInfo",           "hybrid", {"handle": "392585550772135"}),
    ("GroupAbout",          "hybrid", {"handle": "392585550772135"}),
]
...
def test_manual_mode_removed():
    """The scroll-driven `manual` mode was removed — hybrid is the only mode.
    Constructing any endpoint with mode='manual' must now raise."""
    with pytest.raises(ValueError, match="Unsupported mode"):
        Query(
            endpoint="UserTimeline", mode="manual",
            query={"handle": "zuck", "start_date": "2024-01-01", "end_date": "2024-02-01"},
            params={},
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("GroupTimeline", "hybrid", {"handle": "392585550772135"}),
