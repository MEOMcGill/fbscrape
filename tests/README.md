...
(UserTimeline, Search), one public group (GroupTimeline),
...
| `FBSCRAPE_RUN_E2E=1 pytest -m e2e` | full CLI scrape → flatten / download-media / in-scrape media (opt-in) |
...
them rather than failing. **e2e tests additionally require `FBSCRAPE_RUN_E2E=1`** —
they drive the real CLI against live Facebook, so they are opt-in to avoid
scraping by accident. Without the flag every e2e test skips before its body runs.
...
**e2e/** — CLI smoke, opt-in via `FBSCRAPE_RUN_E2E=1`. Runs
`python -m fbscrape.cli` as a subprocess. `test_scrape_endpoints.py` is
parametrized over **all endpoints** — each scrapes a small bounded window
(capped via `--max-paginations` / `--max-results`) then flattens the output to
parquet, asserting the table is non-empty and carries that endpoint's
identifying column. `test_scrape_then_download_media.py` and
`test_scrape_with_streaming_media.py` cover the media pipelines (post-scrape
download, and in-scrape `--download-media --media-manifest` with manifest
drain). Together they confirm wire-format compatibility between the scrape
output and the post-processing commands.
...
4. Add `tests/integration/test_<new_endpoint>.py` mirroring the matching
   integration test.
5. Add an `EndpointE2E(...)` entry to `ENDPOINTS` in
   `tests/e2e/test_scrape_endpoints.py` (scrape args + the identifying column
   the flattened parquet must carry).
6. Update `EXPECTED_KEYS` in `tests/unit/test_query_registry.py`
